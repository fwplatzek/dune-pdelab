...
#ifndef DUNE_PDELAB_IMEX_ONESTEP_PRESTAGEENGINE_HH
#define DUNE_PDELAB_IMEX_ONESTEP_PRESTAGEENGINE_HH
...
namespace Dune {
  namespace PDELab {
    ...
    */
    ...
    class IMEXOneStepLocalPreStageAssemblerEngine
    ...
    public:
      ...
      /**
         ...
         creates this engine
      */
      IMEXOneStepLocalPreStageAssemblerEngine(LocalAssembler & la_)
        : BaseT(la_),
      ...
      { return lae0->requireSkeleton() || lae1->requireSkeleton() || lae2->requireSkeleton(); }
      //! @}
      ...
      //! assembling.
      void setConstResiduals(Residual & const_residual_0_, Residual & const_residual_1_, Residual & const_residual_2_){
        ...
        setLocalAssemblerEngineDT2(la.la2.localResidualAssemblerEngine(*const_residual_2,*((*solutions)[0])));
      }
      ...
      {
        la.la2.setTime(la.time+la.osp_method->d(la.stage)*la.dt);
      }
      ...
      void preAssembly()
      {
        ...
        *const_residual_2 = 0.0;
        ...
        for (int i=0; i<la.stage; ++i){
          ...
          eb[i] = la.osp_method->eb(la.stage,i);
          ed[i] = la.osp_method->ed(i);
          do0[i] = ( std::abs(b[i]) > 1E-6 );
          do1[i] = ( std::abs(eb[i]) > 1E-6 );
          do2[i] = ( std::abs(a[i]) > 1E-6 );
        }
        ...
        la.la2.preStage(la.time+la.osp_method->d(la.stage)*la.dt,la.stage);
      }
      ...
      void assembleUVVolume(const EG & eg, const LFSU & lfsu, const LFSV & lfsv)
      {
        for (int s=0; s<la.stage; ++s){
          ...
          la.la1.setTime(la.time+ed[s]*la.dt);
          la.la2.setTime(la.time+d[s]*la.dt);

          lae0->setSolution(*((*solutions)[s]));
          lae1->setSolution(*((*solutions)[s]));
          lae2->setSolution(*((*solutions)[s]));

          lae0->loadCoefficientsLFSUInside(lfsu);
          lae1->loadCoefficientsLFSUInside(lfsu);
          lae2->loadCoefficientsLFSUInside(lfsu);
          ...
          if(do1[s]){
            la.la1.setWeight(eb[s]*la.dt_factor1);
            lae1->assembleUVVolume(eg,lfsu,lfsv);
          }

          if(do2[s]){
            la.la2.setWeight(a[s]*la.dt_factor2);
      ...
      void assembleVProcessor(const IG & ig, const LFSV_S & lfsv_s)
      {
        for (int s=0; s<la.stage; ++s){
          ...
          if(do2[s]){
            la.la2.setWeight(a[s]*la.dt_factor1);
                                     ...
                                     const LFSV_C & lfsv_c)
      {
        for (int s=0; s<la.stage; ++s){
          ...
          if(do2[s]){
            la.la2.setWeight(a[s]*la.dt_factor2);
            lae2->assembleVEnrichedCoupling(ig,lfsv_s,lfsv_n,lfsv_c);
          }
      ...
      void assembleUVVolumePostSkeleton(const EG & eg, const LFSU & lfsu, const LFSV & lfsv)
      {
        for (int s=0; s<la.stage; ++s){
          ...
          if(do2[s]){
            la.la2.setWeight(a[s]*la.dt_factor2);
            lae2->assembleUVVolumePostSkeleton(eg,lfsu,lfsv);
          }
      ...
      void assembleVVolumePostSkeleton(const EG & eg, const LFSV & lfsv)
      {
        for (int s=0; s<la.stage; ++s){
          ...
          if(do2[s]){
            la.la2.setWeight(a[s]*la.dt_factor2);
            lae2->assembleVVolumePostSkeleton(eg,lfsv);
          }
        }
      }
      //! @}
    ...
    }; // end class IMEXOneStepLocalPreStageAssemblerEngine
